...
$donnees['titre_page'] = "Page jeux";

if (isset($_GET['n'])) // Ce paramètre est spécifié ?
{
 $n = $_GET['n']; // Récupération de paramètres de type $_GET
 $requete = "SELECT projet_fiche.*,tranche_age,nom FROM projet_fiche,projet_age,projet_theme WHERE projet_fiche.id_age = projet_age.id AND projet_fiche.id_theme = projet_theme.id AND id_type=1 AND id_age=$n;";
...
{
  $requete = "SELECT projet_fiche.*,tranche_age,nom FROM projet_fiche,projet_age,projet_theme WHERE projet_fiche.id_age = projet_age.id AND projet_fiche.id_theme = projet_theme.id AND id_type=1;";
  $reponse = $pdo->prepare($requete);
  $reponse->execute();
  // récupérer tous les enregistrements dans un tableau
  $enregistrements = $reponse->fetchAll();
...
<main id="fiches">
  ...
  <?php if (isset($_GET['n']))
   {
   ?>
   <h2>Jeux <?php echo $enregistrements[0]['tranche_age'] ?></h2>
   <?php
    }
    else
    {?>
